...
public class TicTacToe {
    ...
    private boolean check_win_rows() {
        ...
        for (int i = 0; i < this.rows; i++) {
            if (this.board[i][0].equals (this.board[i][1]) && this.board[i][0].equals(this.board[i][2]) && this.board[i][0].length() > 0) {
                result = true;
                break;
            }
        }
    ...
    private boolean check_win_cols() {
        boolean result = false;
        for (int j = 0; j < this.cols; j++) {
            if (this.board[0][j].equals(this.board[1][j]) && this.board[2][j].equals(this.board[0][j]) && this.board[0][j].length() > 0) {
                result = true;
                break;
            }
        }
    ...
    private boolean check_win_cross() {
        boolean result = false;
        if (this.board[0][0].equals(this.board[1][1]) && this.board[2][2].equals(this.board[1][1]) && this.board[0][0].length() > 0) {
            result = true;
        }
        if (this.board[2][0].equals(this.board[1][1]) && this.board[1][2].equals(this.board[1][1]) && this.board[2][0].length() > 0) {
            result = true;
        }
        return result;
    }
